...
func TestListOnlineDisks(t *testing.T) {
	...
	for i, test := range testCases {
		...
		t.Run(fmt.Sprintf("case-%d", i), func(t *testing.T) {
			...
			partsMetadata, errs := readAllFileInfo(ctx, erasureDisks, nil, bucket, object, "", false)
...
func TestListOnlineDisksSmallObjects(t *testing.T) {
	...
	for i, test := range testCases {
		...
		t.Run(fmt.Sprintf("case-%d", i), func(t *testing.T) {
			...
			partsMetadata, errs := readAllFileInfo(ctx, erasureDisks, nil, bucket, object, "", true)
			fi, err := getLatestFileInfo(ctx, partsMetadata, z.serverPools[0].sets[0].defaultParityCount, errs)
			if err != nil {
				t.Fatalf("Failed to getLatestFileInfo %v", err)
			}
...
func TestDisksWithAllParts(t *testing.T) {
	...
	_, errs := readAllFileInfo(ctx, erasureDisks, nil, bucket, object, "", false)
