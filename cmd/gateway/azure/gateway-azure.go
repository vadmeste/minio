...
const (
	// To change the upload chunk size, set the environmental variable MINIO_AZURE_CHUNK_SIZE_MB with a integer value between 0 and 100
...
func (g *Azure) NewGatewayLayer(creds auth.Credentials) (minio.ObjectLayer, error) {
	...
	if azureUploadChunkSize <= 0 || azureUploadChunkSize > 100 {
		return nil, fmt.Errorf("MINIO_AZURE_CHUNK_SIZE_MB should be an integer value between 0 and 100")
	}

	azureUploadConcurrency, err = env.GetInt("MINIO_AZURE_UPLOAD_CONCURRENCY", 1)
	if err != nil {
		return nil, err
	}
