...
func registerAdminRouter(router *mux.Router, enableConfigOps, enableIAMOps bool) {
	...
	// ObjectLayerInfo operations
	adminRouter.Methods(http.MethodGet).Path(adminAPIVersionPrefix + "/objectlayerinfo").HandlerFunc(httpTraceAll(adminAPI.ObjectLayerInfoHandler))
