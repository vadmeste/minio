	...
	scan:
		...
		go func(disk StorageAPI, bucket string) {
			...
			if cache.Info.Name != bucket {
				logger.LogIf(ctx, fmt.Errorf("cache name mismatch: %s != %s", cache.Info.Name, bucket))
			...
			started := scanMgr.markBucketScanStarted(scanID, bucket, cache.Info.NextCycle)
			if !started {
				return
			}
			globalScannerMetrics.bucketScanStarted(bucket, cache.Info.NextCycle)
			...
			globalScannerMetrics.bucketScanFinished(bucket)
			scanMgr.markBucketScanDone(scanID, bucket)
