...
func (c *ClusterReplMgr) AddPeerClusters(ctx context.Context, arg madmin.CRAdd) error {
	...
	for i, v := range arg.Clusters {
		admClient, err := getAdminClient(v)
		if err != nil {
...
// getS3Client - NOTE: ensure to take at least a read lock on ClusterReplMgr
// before calling this.
func (c *ClusterReplMgr) getS3Client(ctx context.Context, deploymentID string) (*minioClient.Client, error) {
	creds, err := c.getPeerCreds()
	if err != nil {
		return nil, err
	}
...
func (c *ClusterReplMgr) getAdminClient(ctx context.Context, deploymentID string) (*madmin.AdminClient, error) {
	...
	ep, _ := url.Parse(peer.Endpoint)
	return madmin.New(ep.Host, creds.AccessKey, creds.SecretKey, ep.Scheme == "https")
}
