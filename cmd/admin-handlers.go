...
func (a adminAPIHandlers) GetConfigKeysHandler(w http.ResponseWriter, r *http.Request) {
	ctx := newContext(r, w, "GetConfigKeys")
...
func (a adminAPIHandlers) SetConfigKeysHandler(w http.ResponseWriter, r *http.Request) {
	...
	ctx := newContext(r, w, "SetConfigKeys")
	...
	for k, _ := range queries {
		encryptedElem, err := base64.StdEncoding.DecodeString(queries.Get(k))
		if err != nil {
			logger.LogIf(ctx, err)
			writeErrorResponseJSON(w, ErrAdminConfigBadJSON, r.URL)
			return
		}
		elemBytes, err := madmin.DecryptServerConfigData(password, bytes.NewBuffer([]byte(encryptedElem)))
		if err != nil {
			logger.LogIf(ctx, err)
			writeErrorResponseJSON(w, ErrAdminConfigBadJSON, r.URL)
			return
