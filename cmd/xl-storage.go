...
func (s *xlStorage) ReadVersion(volume, path, versionID string, checkDataDir bool) (fi FileInfo, err error) {
	buf, err := s.ReadAll(volume, pathJoin(path, xlStorageFormatFile))
	if err != nil {
	...
	if fi.DataDir != "" && checkDataDir {
		if _, err = s.StatVol(pathJoin(volume, path, fi.DataDir, slashSeparator)); err != nil {
