...
func recursiveDeleteIfEmpty(dir, base string) {
	...
	if err != nil {
		if debug {
			log.Fatal("recursiveDeleteIfEmpty:", err)
		}
		return
	}
	if !empty {
	...
	err = os.Remove(dir)
	if err == nil {
		recursiveDeleteIfEmpty(filepath.Dir(dir), base)
	}
}
...
func removeObjectAndEmptyParents(filename string, base string) {
	...
	recursiveDeleteIfEmpty(filepath.Dir(filename), base)
}
...
func mainAction(c *cli.Context) error {
	...
	if basePath == "" || !strings.HasPrefix(basePath, "/") {
		log.Fatal("--base-path should not be empty and should have a leading slashe e.g. /mnt/export0/bucket/")
	}
	dryRun = c.Bool("dry-run")
	verbose = c.Bool("verbose")
	olderThan = c.Duration("older-than")
	if olderThan <= 0 {
		log.Fatal("--older-than needs to be non zero positive value")
	}
	...
	for _, arg := range c.Args() {
		if !wildcard.MatchSimple(basePath, arg) {
			log.Fatal("All arguments should satisfy --base-path")
		}
	}
		...
		go func(arg string) {
			...
			err := filepath.Walk(arg, func(path string, info fs.FileInfo, err error) error {
				...
				if latestModTime > refModTime {
					...
					if e != nil {
						if debug {
							log.Fatal("calculateNewestVersion():", err)
						}
						return nil
					}
				}
