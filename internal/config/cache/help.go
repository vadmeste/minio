...
var (
	...
	Help = config.HelpKVS{
		...
		config.HelpKV{
			Key:         PrefixDepth,
			Type:        "count",
			Description: "prefix depth number beyond which MinIO will automatically create hardlinks cache to gain from lookup performance" + defaultHelpPostfix(PrefixDepth),
			Optional:    true,
		},
		config.HelpKV{
			Key:         Endpoint,
